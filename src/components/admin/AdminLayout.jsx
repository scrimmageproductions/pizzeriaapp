...
import { LayoutGrid, Menu, Pizza, Settings, ShoppingBag, Store, X } from "lucide-react";
import { useShopState } from "../../context/ShopContext";

const NAV_ITEMS = [
  { to: "/admin", end: true, label: "Overview", icon: Store },
  { to: "/admin/menu", label: "Menu Manager", icon: ShoppingBag },
  { to: "/admin/settings", label: "Store Settings", icon: Settings },
  { to: "/admin/kds", label: "Order KDS", icon: LayoutGrid },
];
      ...
      >
        <div className="flex items-center justify-between gap-2 border-b border-gray-100 px-5 py-5">
          <button onClick={() => navigate("/")} className="flex items-center gap-2.5">
            ...
            <div className="text-left">
              ...
              <p className="text-xs leading-tight text-gray-500">Owner Dashboard</p>
            </div>
          </button>
          <button
        ...
        <div className="flex items-center gap-3 border-b border-gray-100 px-5 py-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-gray-500">pizzaplug.com/{shop.slug}</p>
          </div>
        </div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${
                    isActive ? "bg-[#E31837] text-white shadow-sm" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                  }`
                }
              >
                {({ isActive }) => (
