...
export default function CateringPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">📅 Catering & Events</h1>
        <p className="text-sm text-gray-500 dark:text-white/40">Big future orders, held off today's KDS until the kitchen actually needs to see them.</p>
      </div>
      ...
      <Card title="New Catering Quote" description="Build the invoice, then collect a 50% deposit to lock in the date." icon={CalendarClock}>
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <FormField label="Event Date & Time" className="sm:col-span-2">
            <input
              ...
              className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm outline-none transition focus:border-[#E31837] focus:ring-2 focus:ring-[#E31837]/20 dark:border-white/15 dark:bg-white/5 dark:text-white dark:[color-scheme:dark]"
            />
          </FormField>
        </div>

        <div className="mt-4">
          <p className="mb-2 text-sm font-semibold text-gray-800 dark:text-white/80">Menu Items</p>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {items.map((item) => (
              <button
                key={item.id}
                onClick={() => addToTicket(item)}
                className="flex flex-col items-start gap-0.5 rounded-xl border border-gray-200 p-3 text-left text-sm transition hover:border-[#E31837] hover:bg-[#E31837]/5 dark:border-white/10"
              >
                <span className="font-bold text-gray-900 dark:text-white">{item.name}</span>
                <span className="text-xs text-gray-400 dark:text-white/30">{formatCurrency(item.price)}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {ticket.length > 0 && (
          <div className="mt-4 space-y-2 rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-white/10 dark:bg-white/5">
            {ticket.map((t) => (
              <div key={t.id} className="flex items-center gap-2">
                <span className="min-w-0 flex-1 truncate text-sm font-semibold text-gray-800 dark:text-white/80">{t.name}</span>
                <button
                  onClick={() => updateQty(t.id, t.qty - 1)}
                  className="flex h-6 w-6 items-center justify-center rounded-full bg-white text-gray-600 shadow-sm hover:bg-gray-100 dark:bg-white/10 dark:text-white/70 dark:shadow-none dark:hover:bg-white/20"
                >
                  <Minus size={12} />
                </button>
                <span className="w-8 text-center text-sm font-bold dark:text-white">{t.qty}x</span>
                <button
                  onClick={() => updateQty(t.id, t.qty + 1)}
                  className="flex h-6 w-6 items-center justify-center rounded-full bg-white text-gray-600 shadow-sm hover:bg-gray-100 dark:bg-white/10 dark:text-white/70 dark:shadow-none dark:hover:bg-white/20"
                >
                ...
                <span className="w-16 shrink-0 text-right text-sm font-bold text-gray-900 dark:text-white">{formatCurrency(t.price * t.qty)}</span>
              </div>
            ))}
            <div className="flex justify-between border-t border-gray-200 pt-2 text-sm text-gray-500 dark:border-white/10 dark:text-white/50">
      ...
      {inKitchen.length > 0 && (
        <Card title="Currently in the Kitchen" description="Catering orders that have crossed the 2-hour mark." icon={ShoppingBag}>
          <div className="space-y-2">
            {inKitchen.map((o) => (
              <div key={o.id} className="flex items-center justify-between rounded-xl border border-purple-200 bg-purple-50 px-4 py-3 dark:border-purple-500/30 dark:bg-purple-500/10">
                <div>
                  <p className="text-sm font-bold text-gray-900 dark:text-white">{o.customerName}</p>
                  <p className="text-xs text-purple-600 dark:text-purple-300">Due at {formatClockTime(o.eventAt)}</p>
                </div>
            ...
            {upcoming.map((o) => (
              <div key={o.id} className="rounded-xl border border-gray-100 p-3 dark:border-white/10">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white">{o.customerName}</p>
                    <p className="text-xs text-gray-500 dark:text-white/40">{o.customerEmail}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-gray-900 dark:text-white">{formatEventAt(o.eventAt)}</p>
                    <p className="text-xs text-gray-400 dark:text-white/30">Kitchen alert at {formatClockTime(o.eventAt - TWO_HOURS_MS)}</p>
                  </div>
                </div>
                <div className="mt-2 flex flex-wrap items-center justify-between gap-2 border-t border-dashed border-gray-100 pt-2 text-xs text-gray-500 dark:border-white/10 dark:text-white/40">
